...
async def variable(event):
    ...
    vinfo = reply.text
    zed = await edit_or_reply(event, "**⎉╎جـاري اضـافة الفـار إلى بـوتك ...**")
...
async def custom_rep(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        variable = "TZ"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار المنطقـــة الزمنيـــة غيـر موجـود ❌**\n**⎉╎لـ اضـافته فقـط استخـدم الامـر : ↶**\n `.وقت` واسـم الدولـة \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
...
async def variable(event):
    ...
    zed = await edit_or_reply(event, "**⎉╎جـاري إعـداد المنطقـــة الزمنيـــة لـ تيبثـون 🌐...**")
    # All Rights Reserved for "Tepthon" "باقر"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن":
        ...
        if gvarstatus("TZ") is not None:
            await zed.edit("**⎉╎تم تغييـر المنطقـــة الزمنيـــة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعــادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ⏰ ▬▭ ...**".format(input_str))
        else:
            await zed.edit("**⎉╎تم إضـافــة المنطقـــة الزمنيـــة .. بنجـاح ☑️**\n**⎉╎المضـاف إليـــة : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعــادة تشغيـل بـوت تيبثـون يستغـرق الأمر 2-1 دقيقـة ⏰ ▬▭ ...**".format(input_str))
        addgvar("TZ", viraq)
...
async def cmd(zed):
    await edit_or_reply(zed, ZelzalVP_cmd)
...
async def cmd(zed):
    await edit_or_reply(zed, ZelzalTZ_cmd)
